...
void vMBPortSerialEnable(BOOL xRxEnable, BOOL xTxEnable)
{
    ...
    if(xRxEnable)
    {
        UART_ENABLE_INT(MODBUS_UART_PORT, UART_INTEN_RDAIEN_Msk);
    }
    else
    {
        UART_DISABLE_INT(MODBUS_UART_PORT, UART_INTEN_RDAIEN_Msk);
    }
    if(xTxEnable)
    {
...
void vMBPortClose(void)
{
	#if 1
    /* Disable Interrupt */
    UART_DISABLE_INT(MODBUS_UART_PORT, UART_INTEN_RDAIEN_Msk | UART_INTEN_THREIEN_Msk);
